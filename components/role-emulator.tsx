...
export function RoleEmulator({ addLog, currentTest }: RoleEmulatorProps) {
  ...
  const handleTakeOrder = (orderId: number) => {
    ...
    if (order) {
      ...
      setAssignedOrders([...assignedOrders, { ...order, courierId: 100, status: "taken_from_exchange" }])
      handleAction("courier", "take_order", { order_id: orderId, courier_id: 100 })
    }
  }

  const handlePlaceInCell = (orderId: number) => {
    setAssignedOrders(
      assignedOrders.map((order) => (order.id === orderId ? { ...order, status: "placed_in_cell" } : order)),
    )
    handleAction("courier", "place_in_cell", { order_id: orderId })
  }
          ...
          <TabsContent value="courier" className="mt-0">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <h3 className="font-semibold mb-3">{t.courier.assignedOrders}</h3>
                  <div className="border rounded-lg overflow-hidden">
                    <Table>
                      <TableHeader>
                        <TableRow>
                        ...
                        {assignedOrders.map((order) => (
                          <TableRow key={order.id}>
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  order.status === "taken_from_exchange"
                                    ? "default"
                                    : order.status === "placed_in_cell"
                                      ? "secondary"
                                      : "secondary"
                                }
                              >
                            ...
                            <TableCell className="space-x-2">
                              <Button
                                size="sm"
                                onClick={() => handlePlaceInCell(order.id)}
                                disabled={order.status === "placed_in_cell"}
                                className={highlightedAction === "place_in_cell" ? "animate-pulse" : ""}
                              >
                                {t.courier.placeInCell}
                              </Button>
                              <Button
                                size="sm"
                                variant="outline"
